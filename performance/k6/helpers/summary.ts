...
export function generateSummary(scenarioName: string, testType: string) {
  return function (data: Record<string, unknown>) {
    ...
    return {
      ...
      [htmlFile]: htmlReport(data, scenarioName, testType),
      [jsonFile]: jsonSummary(data, scenarioName, testType, timestamp),
    };
  };
}
...
function htmlReport(data: Record<string, unknown>, scenarioName: string, testType: string): string {
  ...
  const overallBadge =
    thresholdRows.length === 0
      ? `<span class="badge badge-info">No Thresholds</span>`
      : allPass
...
<div class="cards">
  ...
  <div class="card"><div class="label">Max VUs</div><div class="value">${maxVUs !== null ? maxVUs.toFixed(0) : '-'}</div></div>
</div>
